...
// Removed legacy SDK nonce

pub struct StoicPersonality {
    pub allowed_interactions: HashSet<String>,
    ...
    pub agent: Agent<rig::providers::openai::CompletionModel>,
}
...
5. End with a stoic analysis based on actual market indicators:
   ...
   - Development activity"#.to_string(),
            ...
            agent: rig::providers::openai::Client::from_env().agent("gpt-4").build(),
        }
    }
    ...
    pub async fn generate_trade_tweet(
        &self,
        agent: &crate::agents::TradingAgentSystem,
        trade_details: &str,
...
5. Use cashtags for token symbols"#,
    ...
    fn postprocess_tweet(&self, tweet: String) -> String {
        ...
        if !processed.contains("#StoicTrading") {
            processed.push_str("\n\n#StoicTrading #OneChain #AlgoTrading");
        }
...
5. Stay under 280 characters"#,
    ...
    pub async fn analyze_state(&self, state_data: &str) -> crate::analysis::Analysis {
        let prompt = format!("{} Analyze market state:\n{}", 
            self.base_prompt,
            state_data
        );
        
        let response_text = self.agent.prompt(&prompt).await.unwrap_or_default();
        if response_text.to_lowercase().contains("buy") {
            crate::analysis::Analysis::Buy
        } else if response_text.to_lowercase().contains("sell") {
            crate::analysis::Analysis::Sell
        } else {
            crate::analysis::Analysis::Hold
        }
    }
}
