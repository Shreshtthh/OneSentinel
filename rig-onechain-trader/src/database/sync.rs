...
use crate::execution::OneChainExecutor;
use anyhow::Result;
...
pub struct DataSyncService {
    ...
    executor: Arc<OneChainExecutor>,
    personality: StoicPersonality,
    wallet: Arc<Ed25519KeyPair>,
    sync_interval: u64,
}
...
impl DataSyncService {
    ...
    pub async fn sync_market_data(&self) -> Result<()> {
        ...
        for trend in trends {
            ...
            self.db.collection("token_states").insert_one(state).await?;
            ...
            if let Ok(analysis) = self.trading_strategy.analyze_opportunity_str(&prompt, 1.0).await {
...
- Momentum: {}",
                                        ...
                                        &crate::market_data::MarketData {
                                            token: trend.metadata.clone(),
                                            market_cap: trend.metadata.market_cap,
                                            volatility: 0.1,
                                            social_sentiment: Some(0.5),
                                            technical_indicators: crate::market_data::TechnicalIndicators {
                                                rsi_14: Some(50.0), macd: Some(0.0), ma_50: Some(0.0), ma_200: Some(0.0),
                                            },
                                        }
                                    ).await?;
